...
public class MorphToken {
    ...
    public String getLemma() {
        ...
        if (wordForms != null && wordForms.size() > 0) {
            if (wordForms.size() == 1) 
                res = (String)com.pullenti.n2j.Utils.notnull(wordForms.get(0).normalFull, wordForms.get(0).normalCase);
            if (res == null && !charInfo.isAllLower()) {
                for(MorphWordForm m : wordForms) {
                    if (m._getClass().isProperSurname()) {
                        String s = (String)com.pullenti.n2j.Utils.notnull(m.normalFull, com.pullenti.n2j.Utils.notnull(m.normalCase, ""));
            ...
            if (res == null) {
                ...
                for(MorphWordForm m : wordForms) {
                    if (best == null) 
                        best = m;
                    else if (compareForms(best, m) > 0) 
                        best = m;
                }
                res = (String)com.pullenti.n2j.Utils.notnull(best.normalFull, best.normalCase);
            }
        }
        if (res != null) {
            ...
            else if (LanguageHelper.endsWith(res, "АНИЙ") && com.pullenti.n2j.Utils.stringsEq(res, term)) {
        ...
        return (String)com.pullenti.n2j.Utils.notnull(term, "?");
    }
    ...
    private int compareForms(MorphWordForm x, MorphWordForm y) {
        String vx = (String)com.pullenti.n2j.Utils.notnull(x.normalFull, x.normalCase);
        String vy = (String)com.pullenti.n2j.Utils.notnull(y.normalFull, y.normalCase);
        if (com.pullenti.n2j.Utils.stringsEq(vx, vy)) 
            return 0;
        if (com.pullenti.n2j.Utils.isNullOrEmpty(vx)) 
            return 1;
        if (com.pullenti.n2j.Utils.isNullOrEmpty(vy)) 
            return -1;
