...
public class Referent {
    ...
    public int getIntValue(String attrName, int defValue) {
        ...
        if (com.pullenti.n2j.Utils.isNullOrEmpty(str)) 
            return defValue;
        int res;
        com.pullenti.n2j.Outargwrapper<Integer> inoutarg2766 = new com.pullenti.n2j.Outargwrapper<>();
        boolean inoutres2767 = com.pullenti.n2j.Utils.parseInteger(str, inoutarg2766);
        res = (inoutarg2766.value != null ? inoutarg2766.value : 0);
        if (!inoutres2767) 
            return defValue;
        return res;
    }
    ...
    public void addOccurence(TextAnnotation anno) {
        ...
        if (anno.getOccurenceOf() != this && anno.getOccurenceOf() != null) 
            anno = TextAnnotation._new2769(anno.beginChar, anno.endChar, anno.sofa);
        if (m_Occurrence == null) 
            m_Occurrence = new java.util.ArrayList<>();
    ...
    public Referent clone() {
        ...
        for(Slot r : getSlots()) {
            Slot rr = Slot._new2770(r.getTypeName(), r.getValue(), r.getCount());
            rr.setOwner(res);
            res.getSlots().add(rr);
        }
        return res;
    ...
    public void deserialize(com.pullenti.n2j.Stream stream, java.util.ArrayList<Referent> all, SourceOfAnalysis sofa) throws java.io.IOException {
        ...
        m_Occurrence = new java.util.ArrayList<>();
        for(int i = 0; i < cou; i++) {
            TextAnnotation a = TextAnnotation._new2772(sofa, this);
            m_Occurrence.add(a);
