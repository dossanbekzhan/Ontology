...
public class SourceOfAnalysis {
    ...
    private String doCrLfCorrection(String txt) {
        ...
        for(i = 0; i < txt.length(); i++) {
            ...
            if (lastChar != '.' && lastChar != ':' && lastChar != ';') {
                ...
                for(int k = j + 1; k < txt.length(); k++) {
                    if (!com.pullenti.n2j.Utils.isWhitespace(txt.charAt(k))) {
        ...
        for(i = 0; i < tmp.length(); i++) {
            ...
            if (j >= tmp.length()) 
                break;
            for(jj = j - 1; jj >= 0; jj--) {
                if (!com.pullenti.n2j.Utils.isWhitespace((lastChar = tmp.charAt(jj)))) 
                    break;
            }
    ...
    public void serialize(com.pullenti.n2j.Stream stream) throws java.io.IOException {
        com.pullenti.ner.core.internal.SerializerHelper.serializeString(stream, getText());
    }
