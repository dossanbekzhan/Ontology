...
public class BusinessFactItem extends com.pullenti.ner.MetaToken {
    ...
    public static void initialize() {
        ...
        for(String s : new String[] {"КУПИТЬ", "ПОКУПАТЬ", "ПРИОБРЕТАТЬ", "ПРИОБРЕСТИ", "ПОКУПКА", "ПРИОБРЕТЕНИЕ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.GET));
        }
        for(String s : new String[] {"КУПИТИ", "КУПУВАТИ", "КУПУВАТИ", "ПРИДБАТИ", "ПОКУПКА", "ПРИДБАННЯ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.GET, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"ПРОДАТЬ", "ПРОДАВАТЬ", "ПРОДАЖА"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.SELL));
        }
        for(String s : new String[] {"ПРОДАТИ", "ПРОДАВАТИ", "ПРОДАЖ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.SELL, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"ФИНАНСИРОВАТЬ", "СПОНСИРОВАТЬ", "ПРОФИНАНСИРОВАТЬ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.FINANCE));
        }
        for(String s : new String[] {"ФІНАНСУВАТИ", "СПОНСОРУВАТИ", "ПРОФІНАНСУВАТИ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.FINANCE, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"ВЛАДЕТЬ", "РАСПОРЯЖАТЬСЯ", "КОНТРОЛИРОВАТЬ", "ПРИНАДЛЕЖАТЬ", "СТАТЬ ВЛАДЕЛЬЦЕМ", "КОНСОЛИДИРОВАТЬ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.HAVE));
        }
        for(String s : new String[] {"ВОЛОДІТИ", "РОЗПОРЯДЖАТИСЯ", "КОНТРОЛЮВАТИ", "НАЛЕЖАТИ", "СТАТИ ВЛАСНИКОМ", "КОНСОЛІДУВАТИ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.HAVE, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"ПРИНАДЛЕЖАЩИЙ", "КОНТРОЛИРУЕМЫЙ", "ВЛАДЕЕМЫЙ", "ПЕРЕЙТИ ПОД КОНТРОЛЬ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new120(s, com.pullenti.ner.business.BusinessFactKind.HAVE, s));
        }
        ...
        for(String s : new String[] {"ЗАКРЫТЬ СДЕЛКУ", "СОВЕРШИТЬ СДЕЛКУ", "ЗАВЕРШИТЬ СДЕЛКУ", "ЗАКЛЮЧИТЬ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.UNDEFINED));
        }
        for(String s : new String[] {"ЗАКРИТИ ОПЕРАЦІЮ", "ЗДІЙСНИТИ ОПЕРАЦІЮ", "ЗАВЕРШИТИ ОПЕРАЦІЮ", "УКЛАСТИ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.UNDEFINED, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"ДОХОД", "ПРИБЫЛЬ", "ВЫРУЧКА"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.PROFIT));
        }
        for(String s : new String[] {"ДОХІД", "ПРИБУТОК", "ВИРУЧКА"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.PROFIT, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"УБЫТОК"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.DAMAGES));
        }
        for(String s : new String[] {"ЗБИТОК"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.DAMAGES, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"СОГЛАШЕНИЕ", "ДОГОВОР"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.AGREEMENT));
        }
        for(String s : new String[] {"УГОДА", "ДОГОВІР"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.AGREEMENT, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"ИСК", "СУДЕБНЫЙ ИСК"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.LAWSUIT));
        }
        for(String s : new String[] {"ПОЗОВ", "СУДОВИЙ ПОЗОВ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.LAWSUIT, com.pullenti.morph.MorphLang.UA));
        }
        for(String s : new String[] {"ДОЧЕРНЕЕ ПРЕДПРИЯТИЕ", "ДОЧЕРНЕЕ ПОДРАЗДЕЛЕНИЕ", "ДОЧЕРНЯЯ КОМПАНИЯ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new118(s, com.pullenti.ner.business.BusinessFactKind.SUBSIDIARY));
        }
        for(String s : new String[] {"ДОЧІРНЄ ПІДПРИЄМСТВО", "ДОЧІРНІЙ ПІДРОЗДІЛ", "ДОЧІРНЯ КОМПАНІЯ"}) {
            m_BaseOnto.add(com.pullenti.ner.core.Termin._new119(s, com.pullenti.ner.business.BusinessFactKind.SUBSIDIARY, com.pullenti.morph.MorphLang.UA));
        }
    }
