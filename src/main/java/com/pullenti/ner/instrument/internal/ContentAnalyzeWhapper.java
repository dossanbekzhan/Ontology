...
public class ContentAnalyzeWhapper {
    ...
    private void _analizeContentWithContainers(FragToken root, java.util.ArrayList<InstrToken1> _lines, int topLevel, FragToken _topDoc) {
        ...
        for(int i = 0; i <= _lines.size(); i++) {
            ...
            if (li == null || (((n < nums.size()) && li == nums.get(n)))) {
                if (blk.size() > 0) {
                    if (fr == null) {
                        fr = FragToken._new1207(blk.get(0).getBeginToken(), blk.get(blk.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT);
                        if (blk.size() == 1) 
                            fr.itok = blk.get(0);
            ...
            if ((n < nums.size()) && li == nums.get(n)) {
                n++;
                fr = FragToken._new1208(li.getBeginToken(), li.getEndToken(), li, li.isExpired);
                root.children.add(fr);
                ...
                if (li.getBeginToken() != li.numBeginToken && li.numBeginToken != null) 
                    fr.children.add(FragToken._new1209(li.getBeginToken(), li.numBeginToken.getPrevious(), com.pullenti.ner.instrument.InstrumentKind.KEYWORD, true, li));
                ...
                if (li.numEndToken != li.getEndToken() && li.numEndToken != null) {
                    if (!li.allUpper && ((((li.hasVerb && names == 0 && li.getEndToken().isCharOf(".:"))) || li.getEndToken().isChar(':')))) 
                        fr.children.add(FragToken._new1207(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
                    else {
                        FragToken frName = FragToken._new1209(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                    ...
                    if (_lines.get(i + 1).allUpper || ((_lines.get(i + 1).getBeginToken().isChar('['))) || _lines.get(i).getEndToken().isChar('.')) {
                        ...
                        FragToken frName = FragToken._new1209(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                        fr.children.add(frName);
                        fr.name = FragToken.getRestoredNameMT(frName);
                        i = correctName(fr, frName, _lines, i);
                        names++;
            ...
            if (li.typ == InstrToken1.Types.EDITIONS && blk.size() == 0 && fr != null) 
                fr.children.add(FragToken._new1207(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.EDITIONS));
            else 
                blk.add(li);
        }
    }
    ...
    private boolean _analizeChapterWithoutKeywords(FragToken root, java.util.ArrayList<InstrToken1> _lines, FragToken _topDoc) {
        ...
        for(int i = 0; i <= _lines.size(); i++) {
            ...
            if ((n < nums.size()) && li == nums.get(n)) {
                ...
                fr = FragToken._new1215(li.getBeginToken(), li.getEndToken(), li);
                childs.add(fr);
                fr.kind = com.pullenti.ner.instrument.InstrumentKind.CHAPTER;
                NumberingHelper.createNumber(fr, li);
                if (li.numEndToken != li.getEndToken() && li.numEndToken != null) {
                    if (li.hasManySpecChars) 
                        fr.children.add(FragToken._new1216(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li));
                    else {
                        FragToken frName = FragToken._new1209(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                        fr.children.add(frName);
                        fr.name = FragToken.getRestoredNameMT(frName);
    ...
    private void _addCommentOrEdition(FragToken fr, InstrToken1 li) {
        if (li.typ == InstrToken1.Types.COMMENT) 
            fr.children.add(FragToken._new1216(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.COMMENT, li));
        else if (li.typ == InstrToken1.Types.EDITIONS) {
            FragToken edt = FragToken._new1216(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.EDITIONS, li);
            fr.children.add(edt);
    ...
    private void _analizeContentWithoutContainers(FragToken root, java.util.ArrayList<InstrToken1> _lines, boolean isSubitem, boolean isPreamble, boolean isKodex) {
        ...
        if (root.kind == com.pullenti.ner.instrument.InstrumentKind.CLAUSE || ((root.kind == com.pullenti.ner.instrument.InstrumentKind.CHAPTER && docTyp == com.pullenti.ner.decree.DecreeKind.CONTRACT))) {
            ...
            if (_lines.get(0).numbers.size() == 0 && docTyp != com.pullenti.ner.decree.DecreeKind.CONTRACT) {
                ...
                for(int ii = 0; ii < _lines.size(); ii++) {
                    ...
                    if (part == null) {
                        part = FragToken._new1223(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CLAUSEPART, parts.size() + 1);
                        parts.add(part);
                    }
            ...
            if (_lines.get(ii).typ == InstrToken1.Types.NOTICE) {
                ...
                if (((li.numbers.size() == 1 && com.pullenti.n2j.Utils.stringsEq(li.numbers.get(0), "1"))) || ((li.numbers.size() == 0 && ii == (_lines.size() - 1)))) {
                    for(int j = ii; j < _lines.size(); j++) {
                        li = _lines.get(j);
                        FragToken not = FragToken._new1216(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NOTICE, li);
                        notices.add(not);
                        if (li.numBeginToken != null && li.getBeginToken() != li.numBeginToken) 
                            not.children.add(FragToken._new1225(li.getBeginToken(), li.numBeginToken.getPrevious(), com.pullenti.ner.instrument.InstrumentKind.KEYWORD, true));
                        if (li.numbers.size() > 0) 
                            NumberingHelper.createNumber(not, li);
                        if (not.children.size() > 0) 
                            not.children.add(FragToken._new1216((com.pullenti.ner.Token)com.pullenti.n2j.Utils.notnull(li.numEndToken, li.getBeginToken()), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li));
                    }
                }
        ...
        if (nums == null) {
            ...
            for(InstrToken1 li : _lines) {
                ...
                if (li.typ == InstrToken1.Types.INDEX) {
                    root.children.add(FragToken._new1216(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDEX, li));
                    last = null;
                    continue;
                }
                if (last != null && last.kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT) 
                    ...
                    root.children.add((last = FragToken._new1216(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li)));
            }
            if (!isPreamble) {
        ...
        for(; i < _lines.size(); i++) {
            ...
            if (li.numEndToken != li.getEndToken() && li.numEndToken != null) 
                ...
                fr.children.add(FragToken._new1231(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true));
            }
            if (blk.size() > 0) {
                fr.setEndToken(blk.get(blk.size() - 1).getEndToken());
        ...
        if (parts.size() > 2) {
            if (parts.get(0).name == null && parts.get(parts.size() - 1).name != null) {
                ...
                if (parts.get(0).children.size() == 0) 
                    parts.get(0).children.add(FragToken._new1216(parts.get(0).getBeginToken(), parts.get(0).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, parts.get(0).itok));
            }
    ...
    private FragToken _createDirectivePart(java.util.ArrayList<InstrToken1> _lines) {
        FragToken res = FragToken._new1207(_lines.get(0).getBeginToken(), _lines.get(_lines.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.DOCPART);
        ...
        if (head.size() > 0) {
            FragToken frHead = FragToken._new1207(head.get(0).getBeginToken(), head.get(head.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT);
        ...
        res.children.add(FragToken._new1235(_lines.get(i).getBeginToken(), _lines.get(i).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.DIRECTIVE, _lines.get(i).value, _lines.get(i)));
            ...
            for(FragToken ch : root.children) {
                if (ch.kind == com.pullenti.ner.instrument.InstrumentKind.CHAPTER || ch.kind == com.pullenti.ner.instrument.InstrumentKind.CLAUSE) {
                    ...
                    if (ch.children.get(i).kind != com.pullenti.ner.instrument.InstrumentKind.CONTENT) {
                        ...
                        sect = FragToken._new1225(ch.children.get(i).getBeginToken(), ch.children.get(i).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.SECTION, true);
                        sect.name = (String)com.pullenti.n2j.Utils.cast(sect.value, String.class);
                        sect.value = null;
                        sect.children.add(FragToken._new1207(sect.getBeginToken(), sect.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME));
                        newChilds.add(sect);
                        ...
                        for(com.pullenti.ner.Token tt = la.getEndToken(); tt != null && tt.beginChar > la.beginChar; tt = tt.getPrevious()) {
                            if (tt.isNewlineBefore()) {
                                ...
                                if ((tt.getPrevious().endChar - la.beginChar) < 20) 
                                    break;
                                nextSect = FragToken._new1225(tt, la.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.SECTION, true);
                                nextSect.name = (String)com.pullenti.n2j.Utils.cast(nextSect.value, String.class);
                                nextSect.value = null;
                                nextSect.children.add(FragToken._new1207(tt, la.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME));
                                break;
                            }
                        }
                        if (nextSect == null) 
    ...
    private static java.util.ArrayList<FragToken> _splitContentByIndents(FragToken fr, int num) {
        if (fr.kind != com.pullenti.ner.instrument.InstrumentKind.CONTENT && fr.kind != com.pullenti.ner.instrument.InstrumentKind.LISTITEM) {
            ...
            if (fr.getBeginToken().isValue("АБЗАЦ", null)) {
                ...
                for(int i = num; i <= next; i++) {
                    res0.add(FragToken._new1240(fr.getBeginToken(), fr.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDENTION, i, true, fr.referents));
                }
                return res0;
            }
        ...
        for(com.pullenti.ner.Token tt = t0; tt != null && tt.endChar <= fr.endChar; tt = tt.getNext()) {
            if (tt.endChar == fr.endChar) {
                ...
                continue;
            FragToken re = FragToken._new1223(t0, tt, com.pullenti.ner.instrument.InstrumentKind.INDENTION, num);
            num++;
    ...
    private void _analizePreamble(FragToken root) {
                    ...
                    if (t.isNewlineBefore()) {
                        if (t.getPrevious().isCharOf(".:;") && t.getPrevious().getPrevious() != null && ((t.getPrevious().getPrevious().isValue("НИЖЕСЛЕДУЮЩИЙ", null) || t.getPrevious().getPrevious().isValue("ДОГОВОР", null)))) {
                            ...
                            if (itt1 != null && !itt1.hasVerb && (itt1.endChar < ch.endChar)) {
                                FragToken clau = FragToken._new1207(t, ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CHAPTER);
                                ...
                                FragToken nam = FragToken._new1209(t, itt1.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, itt1);
                                clau.children.add(nam);
                                clau.name = FragToken.getRestoredNameMT(nam);
                                clau.children.add(FragToken._new1207(itt1.getEndToken().getNext(), ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
        ...
        if (cntCou == 1) {
            ...
            FragToken prm = FragToken._new1207(root.children.get(0).getBeginToken(), root.children.get(i - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.PREAMBLE);
            for(int j = 0; j < i; j++) {
