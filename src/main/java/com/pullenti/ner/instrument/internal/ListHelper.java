...
public class ListHelper {
                        ...
                        if (((int)listBullet) != 0 && ch.getBeginToken().isChar(listBullet)) {
                            for(com.pullenti.ner.Token tt = ch.getBeginToken().getNext(); tt != null && (tt.endChar < ch.endChar); tt = tt.getNext()) {
                                if (tt.getPrevious().isChar('.') && com.pullenti.ner.core.MiscHelper.canBeStartOfSentence(tt)) {
                                    FragToken ch2 = FragToken._new1223(tt, ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDENTION, ch.number);
    ...
    private static int _analizeListItems(java.util.ArrayList<FragToken> chi, int ind) {
        ...
        if (res.getHasChanges() && res.getMultilineChangesValue() != null) {
            ...
            FragToken cit = FragToken._new1207(ci.getBeginToken(), ci.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CITATION);
        ...
        for(LineToken li : lines) {
            ...
            FragToken ch = FragToken._new1223(li.getBeginToken(), li.getEndToken(), (li.isListItem ? com.pullenti.ner.instrument.InstrumentKind.LISTITEM : com.pullenti.ner.instrument.InstrumentKind.CONTENT), li.number);
            ...
            if (chli != null) {
                for(LineToken lt : chli) {
                    ch.children.add(FragToken._new1207(lt.getBeginToken(), lt.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.LISTITEM));
                }
                if (ch.beginChar < ch.children.get(0).beginChar) 
                    ch.children.add(0, FragToken._new1207(ch.getBeginToken(), ch.children.get(0).getBeginToken().getPrevious(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
            }
        }
        return ret;
    }
            ...
            if ((lines.get(i).typ == InstrToken1.Types.LINE && lines.get(i).numbers.size() == 0 && lines.get(i).getBeginToken().isValue("ПРИЛОЖЕНИЯ", "ДОДАТОК")) && lines.get(i + 1).numbers.size() > 0 && lines.get(i).getEndToken().isChar(':')) {
                ...
                for(++i; i < lines.size(); i++) {
                    if (lines.get(i).numbers.size() == 0) {{
                        ...
                        int nn;
                        com.pullenti.n2j.Outargwrapper<Integer> inoutarg1399 = new com.pullenti.n2j.Outargwrapper<>();
                        boolean inoutres1400 = com.pullenti.n2j.Utils.parseInteger(lines.get(i).numbers.get(0), inoutarg1399);
                        nn = (inoutarg1399.value != null ? inoutarg1399.value : 0);
                        if (inoutres1400) 
                            num = nn + 1;
