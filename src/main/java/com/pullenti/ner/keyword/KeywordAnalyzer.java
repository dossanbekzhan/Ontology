...
public class KeywordAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public void process(com.pullenti.ner.core.AnalysisKit kit) {
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext(),cur++) {
            ...
            if (com.pullenti.n2j.Utils.stringsEq(term, "ЕСТЬ")) {
            ...
            if (npt == null) {
                ...
                if (mc.isVerb() && !mc.isPreposition()) {
                    if ((((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(t, com.pullenti.ner.TextToken.class))).isVerbBe()) 
                        continue;
                    if (t.isValue("МОЧЬ", null) || t.isValue("WOULD", null)) 
                        continue;
                    KeywordReferent kref = KeywordReferent._new1450(KeywordType.PREDICATE);
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext(),cur++) {
            ...
            if (t1.isValue("OF", null) && (t1.getWhitespacesAfterCount() < 3) && t1.getNext() != null) {
                ...
                continue;
            KeywordReferent kw2 = (KeywordReferent)com.pullenti.n2j.Utils.cast(t1.getReferent(), KeywordReferent.class);
            if (kw2 == null) 
                continue;
            ...
            com.pullenti.ner.ReferentToken rt1 = com.pullenti.ner.ReferentToken._new709(kwUn, t, t1, t.getMorph());
            kit.embedToken(rt1);
            t = rt1;
        }
    ...
    private static void _addNormals(KeywordReferent kref, java.util.ArrayList<com.pullenti.morph.DerivateGroup> grs, String norm) {
        ...
        for(int k = 0; k < grs.size(); k++) {
            ...
            for(int i = 0; i < (grs.size() - 1); i++) {
                if (_calcRank(grs.get(i)) < _calcRank(grs.get(i + 1))) {
                    ...
                    com.pullenti.n2j.Utils.putArrayValue(grs, i, grs.get(i + 1));
                    com.pullenti.n2j.Utils.putArrayValue(grs, i + 1, gr);
                    ch = true;
                }
            }
            if (!ch) 
        ...
        for(int i = 0; (i < 3) && (i < grs.size()); i++) {
            if (!grs.get(i).isDummy && grs.get(i).words.size() > 0) {
                if (com.pullenti.n2j.Utils.stringsNe(grs.get(i).words.get(0).spelling, norm)) 
    ...
    public static class CompByRank implements java.util.Comparator<com.pullenti.ner.Referent> {
        ...
        public int compare(com.pullenti.ner.Referent x, com.pullenti.ner.Referent y) {
            double d1 = (((com.pullenti.ner.keyword.KeywordReferent)com.pullenti.n2j.Utils.cast(x, com.pullenti.ner.keyword.KeywordReferent.class))).rank;
            double d2 = (((com.pullenti.ner.keyword.KeywordReferent)com.pullenti.n2j.Utils.cast(y, com.pullenti.ner.keyword.KeywordReferent.class))).rank;
            if (d1 > d2) 
                return -1;
            if (d1 < d2) 
                return 1;
    ...
    private com.pullenti.ner.Token _addReferents(com.pullenti.ner.core.AnalyzerData ad, com.pullenti.ner.Token t, int cur, int max) {
        ...
        if (r instanceof com.pullenti.ner.denomination.DenominationReferent) {
            com.pullenti.ner.denomination.DenominationReferent dr = (com.pullenti.ner.denomination.DenominationReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.denomination.DenominationReferent.class);
            KeywordReferent kref0 = KeywordReferent._new1450(KeywordType.REFERENT);
            for(com.pullenti.ner.Slot s : dr.getSlots()) {
                if (com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), com.pullenti.ner.denomination.DenominationReferent.ATTR_VALUE)) 
        ...
        if (r instanceof com.pullenti.ner.money.MoneyReferent) {
            ...
            KeywordReferent kref0 = KeywordReferent._new1450(KeywordType.OBJECT);
            kref0.addSlot(KeywordReferent.ATTR_NORMAL, mr.getCurrency(), false, 0);
            com.pullenti.ner.ReferentToken rt0 = new com.pullenti.ner.ReferentToken(ad.registerReferent(kref0), t, t, null);
            t.kit.embedToken(rt0);
            return rt0;
        }
        if (com.pullenti.n2j.Utils.stringsEq(r.getTypeName(), "DATE") || com.pullenti.n2j.Utils.stringsEq(r.getTypeName(), "DATERANGE") || com.pullenti.n2j.Utils.stringsEq(r.getTypeName(), "BOOKLINKREF")) 
            return t;
        ...
        KeywordReferent kref = KeywordReferent._new1450(KeywordType.REFERENT);
        String norm = null;
        if (com.pullenti.n2j.Utils.stringsEq(r.getTypeName(), "GEO")) 
            norm = r.getStringValue("ALPHA2");
        if (norm == null) 
            norm = r.toString(true, new com.pullenti.morph.MorphLang(null), 0);
        if (norm != null) 
