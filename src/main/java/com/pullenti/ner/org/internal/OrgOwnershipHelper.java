...
public class OrgOwnershipHelper {
     ...
     * @return 
     */
    public static boolean canBeHigher(com.pullenti.ner.org.OrganizationReferent higher, com.pullenti.ner.org.OrganizationReferent lower, boolean robust) {
        if (higher == null || lower == null || higher == lower) 
            return false;
        if (lower.getOwner() != null) 
            return false;
        com.pullenti.ner.org.OrganizationKind hk = higher.getKind();
        com.pullenti.ner.org.OrganizationKind lk = lower.getKind();
        if (higher.canBeEquals(lower, com.pullenti.ner.Referent.EqualType.WITHINONETEXT)) 
            return false;
        if (lower.getHigher() == null && lower.findSlot(com.pullenti.ner.org.OrganizationReferent.ATTR_HIGHER, null, true) != null) 
            return false;
        java.util.Collection<String> hTyps = higher.getTypes();
        java.util.Collection<String> lTyps = lower.getTypes();
        if (hk != com.pullenti.ner.org.OrganizationKind.BANK) {
            for(String v : hTyps) {
                if (lTyps.contains(v)) 
                    return false;
            }
        }
        if (hk != com.pullenti.ner.org.OrganizationKind.DEPARTMENT && lk == com.pullenti.ner.org.OrganizationKind.DEPARTMENT) {
            if (_Contains(lTyps, "курс", null) || _Contains(lTyps, "группа", "група")) 
                return hk == com.pullenti.ner.org.OrganizationKind.STUDY || _Contains(hTyps, "институт", "інститут");
            if (_Contains(lTyps, "епархия", "єпархія") || _Contains(lTyps, "патриархия", "патріархія")) 
                return hk == com.pullenti.ner.org.OrganizationKind.CHURCH;
            if (hk == com.pullenti.ner.org.OrganizationKind.UNDEFINED) {
                if (_Contains(hTyps, "управление", "управління")) 
                    return false;
            }
            return true;
        }
        if (lower.containsProfile(com.pullenti.ner.org.OrgProfile.UNIT) || _Contains(lTyps, "department", null)) {
            if (!higher.containsProfile(com.pullenti.ner.org.OrgProfile.UNIT) && lk != com.pullenti.ner.org.OrganizationKind.DEPARTMENT) 
                return true;
        }
        if (_Contains(hTyps, "правительство", "уряд")) {
            if (lk == com.pullenti.ner.org.OrganizationKind.GOVENMENT) 
        ...
        if (hk == com.pullenti.ner.org.OrganizationKind.GOVENMENT) {
            if (lk == com.pullenti.ner.org.OrganizationKind.GOVENMENT) {
                ...
                if (higher.findSlot(com.pullenti.ner.org.OrganizationReferent.ATTR_NAME, "ФЕДЕРАЛЬНОЕ СОБРАНИЕ", true) != null || hTyps.contains("конгресс") || hTyps.contains("парламент")) {
                    if ((lower.findSlot(com.pullenti.ner.org.OrganizationReferent.ATTR_NAME, "СОВЕТ ФЕДЕРАЦИИ", true) != null || lower.findSlot(com.pullenti.ner.org.OrganizationReferent.ATTR_NAME, "ГОСУДАРСТВЕННАЯ ДУМА", true) != null || lower.findSlot(com.pullenti.ner.org.OrganizationReferent.ATTR_NAME, "ВЕРХОВНА РАДА", true) != null) || _Contains(lTyps, "палата", null) || _Contains(lTyps, "совет", null)) 
                        return true;
                }
                if (higher.findSlot(com.pullenti.ner.org.OrganizationReferent.ATTR_NAME, "ФСБ", true) != null) {
                    if (lower.findSlot(com.pullenti.ner.org.OrganizationReferent.ATTR_NAME, "ФПС", true) != null) 
                        return true;
                }
            ...
            else if (lk == com.pullenti.ner.org.OrganizationKind.UNDEFINED) {
            ...
            else if (lk == com.pullenti.ner.org.OrganizationKind.BANK) {
            }
        }
        if (_Contains(hTyps, "министерство", "міністерство")) {
            ...
            if (_Contains(lTyps, "служба", null) && !_Contains(lTyps, "федеральная служба", "федеральна служба")) 
                return true;
            if (lk == com.pullenti.ner.org.OrganizationKind.CULTURE || lk == com.pullenti.ner.org.OrganizationKind.MEDICAL) 
                return true;
        }
        ...
        if (hk == com.pullenti.ner.org.OrganizationKind.STUDY || _Contains(hTyps, "институт", "інститут") || _Contains(hTyps, "академия", "академія")) {
        ...
        if (hk != com.pullenti.ner.org.OrganizationKind.DEPARTMENT) {
            if (((((_Contains(lTyps, "департамент", null) || _Contains(lTyps, "центр", null))) && hk != com.pullenti.ner.org.OrganizationKind.MEDICAL && hk != com.pullenti.ner.org.OrganizationKind.SCIENCE) && !_Contains(hTyps, "центр", null) && !_Contains(hTyps, "департамент", null)) && !_Contains(hTyps, "управление", "управління")) 
                return true;
            if (_Contains(hTyps, "департамент", null) || robust) {
                if (_Contains(lTyps, "центр", null)) 
                    return true;
                if (lk == com.pullenti.ner.org.OrganizationKind.STUDY) 
                    return true;
            }
            ...
            if (hk == com.pullenti.ner.org.OrganizationKind.PARTY || hk == com.pullenti.ner.org.OrganizationKind.FEDERATION) {
                if (_Contains(lTyps, "комитет", "комітет")) 
                    return true;
            }
            if ((lk == com.pullenti.ner.org.OrganizationKind.FEDERATION && hk != com.pullenti.ner.org.OrganizationKind.FEDERATION && hk != com.pullenti.ner.org.OrganizationKind.GOVENMENT) && hk != com.pullenti.ner.org.OrganizationKind.PARTY) {
                if (!_Contains(hTyps, "фонд", null) && hk != com.pullenti.ner.org.OrganizationKind.UNDEFINED) 
                    return true;
            }
        }
        else if (_Contains(hTyps, "управление", "управління") || _Contains(hTyps, "департамент", null)) {
            if (!_Contains(lTyps, "управление", "управління") && !_Contains(lTyps, "департамент", null) && lk == com.pullenti.ner.org.OrganizationKind.DEPARTMENT) 
                return true;
            if (((_Contains(hTyps, "главное", "головне") && _Contains(hTyps, "управление", "управління") && _Contains(lTyps, "управление", "управління")) && !lTyps.contains("главное управление") && !lTyps.contains("головне управління")) && !lTyps.contains("пограничное управление")) 
                return true;
        ...
        else if ((lk == com.pullenti.ner.org.OrganizationKind.GOVENMENT && _Contains(lTyps, "служба", null) && higher.getHigher() != null) && higher.getHigher().getKind() == com.pullenti.ner.org.OrganizationKind.GOVENMENT) 
            return true;
        else if (_Contains(hTyps, "отдел", "відділ") && lk == com.pullenti.ner.org.OrganizationKind.DEPARTMENT && ((_Contains(lTyps, "стол", "стіл") || _Contains(lTyps, "направление", "напрямок")))) 
            return true;
        if (hk == com.pullenti.ner.org.OrganizationKind.BANK) {
            if (higher.getNames().contains("СБЕРЕГАТЕЛЬНЫЙ БАНК")) {
                if (lk == com.pullenti.ner.org.OrganizationKind.BANK && !lower.getNames().contains("СБЕРЕГАТЕЛЬНЫЙ БАНК")) 
                    return true;
            }
        }
        if (lk == com.pullenti.ner.org.OrganizationKind.MEDICAL) {
